...
begin
  ...
  with KeyWords do begin
    ...
    Add(KWR_TYPE_IDENT     );
    Add(KWR_ASSIGNING      );
    { Вот тут как раз нужен только закрывающий ключ, который укажет вложенному ридеру, что надо завершать текущее
      (последнее во вложенной структуре) значение. А для внешнего блока вообще ключи здесь не нужны.}
