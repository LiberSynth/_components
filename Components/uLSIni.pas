...
{ TODO 2 -oVasilyevSM -cuLSIni: Теряется порядок. Если Words загрузить и сохранить, все перемешивается. }
{ TODO 1 -oVasilyevSM -cuLSIni: Ошибка. Загружаем Words.ini и сохраняем в режиме "строки без кавычек". Потом загружаем - ошибка. }
{ TODO 3 -oVasilyevSM -cuLSIni: Нужен формализованный способ выполнять преднастройку для нетипизованных параметров.
  Примерно через вызов функций RegisterParam или просто присвоением. Считывание уже поддерживает такой режим, ему просто
  нужны все параметры, назначенные перед загрузкой. Вот это назначение и надо сделать удобным. }
{ TODO 3 -oVasilyevSM -cuLSIni: Режим AutoSave. В каждом SetAs вызывать в нем SaveTo... Куда - зависит от типа выходного
