...
(******************************************************)
(*                                                    *)
(*  This is the parameters object. It is designed to  *)
(*  work with parameters of the basic used types.     *)
(*                                                    *)
(******************************************************)
...
type
  ...
  public
    ...
    function CheckParam(const _Name: String; _Default: Double): TParam; overload;
...
function TParams.CheckParam(const _Name: String; _Default: Double): TParam;
begin
  Result := GetParam(_Name);
  with Result do
...
function TParams.InsertParam(_Index: Integer; const _Name: String): TParam;
begin
  Result := TParam.Create(_Name);
  if _Index < 0 then _Index := 0;
  if _Index > Count - 1 then _Index := Count - 1;
  Insert(_Index, Result);
end;
...
function TParamsReader.ReadParamName(var _TypeDesignated: Boolean): String;
  ...
  function _LastParamDescription: String;
  begin

    if FParams.Count > 0 then

      with FParams[FParams.Count - 1] do
        Result := Format('%s: %s = ''%s''', [Name, ParamDataTypeToStr(DataType), AsString])
  ...
  begin
    raise EParamsReadException.CreateFmt(SC_ParamRead_UnterminatedParamName, [Result, _LastParamDescription], Position);
  end;

  procedure _RaiseEmpty;
  begin
    raise EParamsReadException.CreateFmt(SC_ParamRead_EmptyParamName, [_LastParamDescription], Position);
  end;
...
procedure TParamsReader.ReadParams(_Param: TParam);
  ...
  function _EndOf: Boolean;
    ...
    begin
      if EndOf then
        raise EParamsReadException.CreateFmt(SC_ParamRead_UnterminatedParam, [_Param.Name], Position);
    end;
